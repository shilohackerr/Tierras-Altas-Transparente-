...
function renderBarChart() {
  ...
  container.innerHTML = data.map(c => {
    const hR = Math.max(6, Math.round((c.recibidoPct  / 100) * MAX_H));
    const hE = Math.max(6, Math.round((c.ejecutadoPct / 100) * MAX_H));
    return `
    <div class="bar-group">
      <div class="bar-wrap">
        <div class="bar"
             style="height:${hR}px; width:45%; background:#1e53e5; align-self:flex-end; display:block; border-radius:4px 4px 0 0;"
             role="img" aria-label="${c.nombre}: Recibido ${c.recibidoPct}%"
             title="Recibido ${c.recibidoPct}%"></div>
        <div class="bar"
             style="height:${hE}px; width:45%; background:#00e64d; align-self:flex-end; display:block; border-radius:4px 4px 0 0;"
             role="img" aria-label="${c.nombre}: Ejecutado ${c.ejecutadoPct}%"
             title="Ejecutado ${c.ejecutadoPct}%"></div>
      </div>
  ...
  container.innerHTML = PRESUPUESTO.todos.rubros.map((r, i) => `
    <div class="progress-item" role="listitem">
      ...
      <div class="progress-bar-bg" role="progressbar" aria-valuenow="${r.pct}" aria-valuemin="0" aria-valuemax="100" aria-label="${r.nombre}: ${r.pct}%">
        <div class="progress-bar-fill" style="width:${r.pct}%; background:${r.color}; clip-path:inset(0 0% 0 0);"></div>
      </div>
    </div>
  `).join('');
}
    ...
    <div class="promesa-card" role="listitem" aria-label="${p.texto} — ${p.estado}, ${p.pct}%">
      ...
      <div class="promesa-avance">
        <div class="promesa-bar-bg" role="progressbar" aria-valuenow="${p.pct}" aria-valuemin="0" aria-valuemax="100">
          <div class="promesa-bar-fill ${p.estado}" style="width:${p.pct}%; clip-path:inset(0 0% 0 0);"></div>
        </div>
