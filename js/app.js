...
function renderBarChart() {
  ...
  const MAX_H = 110; // px — altura máxima de barra
  ...
  container.innerHTML = data.map(c => {
    ...
    <div class="bar-group">
      <div class="bar-wrap">
        <div class="bar" style="height:${hRecibido}px; background:var(--azul-light);"
             role="img" aria-label="${c.nombre}: Recibido ${c.recibidoPct}%"
             title="${c.nombre}: Recibido ${c.recibidoPct}%"></div>
        <div class="bar" style="height:${hEjecutado}px; background:var(--verde);"
