...
function renderBarChart() {
  ...
  container.innerHTML = data.map(c => {
    const hRecibido  = Math.max(4, Math.round((c.recibidoPct  / 100) * MAX_H));
    const hEjecutado = Math.max(4, Math.round((c.ejecutadoPct / 100) * MAX_H));
    return `
    <div class="bar-group">
      <div class="bar-wrap">
        <div class="bar"
             style="height:${hRecibido}px; background:#1e53e5;"
             role="img" aria-label="${c.nombre}: Recibido ${c.recibidoPct}%"
             title="${c.nombre}: Recibido ${c.recibidoPct}%"></div>
        <div class="bar"
             style="height:${hEjecutado}px; background:#00e64d;"
             role="img" aria-label="${c.nombre}: Ejecutado ${c.ejecutadoPct}%"
             title="${c.nombre}: Ejecutado ${c.ejecutadoPct}%"></div>
      </div>
